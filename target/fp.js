...
let Chaincode = class {
   ...
   */
  async Init(stub) {
    console.log('=========== Init: Instantiated / Upgraded face pay chaincode ===========');
    return shim.success();
  }
   ...
   *      "CCNumber" : "44445555677778888",
   *      "DOB" : "2010-01-02",
   *      "CityBorn" : "Mysore"
   * }
   */
  async registerClient(stub, args) {
    console.log('============= START : registerClient ===========');
    console.log('##### createDonor arguments: ' + JSON.stringify(args));

    // args is passed as a JSON string
    let json = JSON.parse(args);
    let key = 'client' + json['FirstName'] + json['LastName'];
    json['docType'] = 'client';

    console.log('##### register client payload: ' + JSON.stringify(json));

    // Check if the client already exists
    let donorQuery = await stub.getState(key);
    if (donorQuery.toString()) {
      throw new Error('##### client - This client already exists: ' + json['FirstName'] + json['LastName']);
    }

    await stub.putState(key, Buffer.from(JSON.stringify(json)));
    console.log('============= END : registerClient ===========');
  }

  /**
   * Retrieves a specfic client
   * 
   * @param {*} stub 
   * @param {*} args 
   */
  async queryClient(stub, args) {
    console.log('============= START : queryClient ===========');
    console.log('##### queryClient arguments: ' + JSON.stringify(args));

    // args is passed as a JSON string
    let json = JSON.parse(args);
    let key = 'client' + json['FirstName'] + json['LastName'];
    console.log('##### queryClient key: ' + key);
   ...
   * @param {*} args 
   */
  async queryAllClients(stub, args) {
    console.log('============= START : queryAllClients ===========');
    console.log('##### queryAllClients arguments: ' + JSON.stringify(args));
 
    let queryString = '{"selector": {"docType": "client"}}';
   ...
   *    "StoreName" : "7eleven",
   *    "Amount" : "10",
   *    "FaceId" : "1232384023u40",
   *    "Email" : "dipinthomas2003@gmail.com"
   * }
   */
  async makePayment(stub, args) {
    console.log('============= START : makePayment ===========');
    console.log('##### makePayment arguments: ' + JSON.stringify(args));

    // args is passed as a JSON string
    let json = JSON.parse(args);
    let key = 'payment' + json['FaceID'];
    json['docType'] = 'payment';

    console.log('##### makePayment donation: ' + JSON.stringify(json));

    // Confirm the NGO exists
    // let ngoKey = 'ngo' + json['ngoRegistrationNumber'];
    // let ngoQuery = await stub.getState(ngoKey);
    // if (!ngoQuery.toString()) {
    //   throw new Error('##### makePayment - Cannot create donation as the NGO does not exist: ' + json['ngoRegistrationNumber']);
    // }

    // Confirm the client exists
    let donorKey = 'client' + json['FirstName'] + json['LastName'];
    let donorQuery = await stub.getState(donorKey);
    if (!donorQuery.toString()) {
      throw new Error('##### makePayment - Cannot create payment as the client does not exist: ' + json['FirstName'] + json['LastName']);
    }
  ...
  async queryTransaction(stub, args) {
    ...
    // args is passed as a JSON string
    let json = JSON.parse(args);
    let key = 'payment' + json['transactionId'];
